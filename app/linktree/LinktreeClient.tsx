...
import { Phone, Mail, Instagram, Globe, MessageCircle, Video } from "lucide-react"
...
export default function LinktreeClient() {
  ...
  const links = [
    ...
    {
      title: "Enviar Email",
      subtitle: "mminmobiliariaserrana@gmail.com",
      href: "mailto:mminmobiliariaserrana@gmail.com",
      icon: Mail,
  ...
  return (
    ...
    >
      <div className="w-full h-56 xs:h-64 sm:h-72 md:h-80 lg:h-96 xl:h-[28rem] relative overflow-hidden rounded-b-3xl shadow-2xl">
        ...
        >
          <source
            src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/recorrido-virtual-ew2dhUGdBBqDUoFqr5glBWcEfCDEeX.mp4"
            type="video/mp4"
          />
        </video>
      ...
      <div className="relative z-10 px-3 xs:px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10">
        ...
        <div className="space-y-3 sm:space-y-4 md:space-y-5 max-w-xs xs:max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto w-full">
          ...
          <Button
            ...
            className="w-full h-auto py-3 sm:py-4 md:py-5 px-4 sm:px-6 md:px-8 bg-purple-600 hover:bg-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] rounded-xl sm:rounded-2xl border-2 border-amber-400/20 hover:border-amber-400/40 group"
          >
            <div className="flex items-center justify-center space-x-3 sm:space-x-4 md:space-x-5 w-full">
              <Video className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 flex-shrink-0 text-amber-200 group-hover:text-amber-100 transition-colors duration-300" />
              <div className="text-left flex-1 min-w-0">
                <div className="font-montserrat font-bold text-sm sm:text-base md:text-lg lg:text-xl truncate group-hover:text-amber-50 transition-colors duration-300">
                  Demo Recorrido Virtual
                </div>
                <div className="font-poppins text-xs sm:text-sm md:text-base opacity-90 truncate text-amber-100 group-hover:text-amber-50 transition-colors duration-300">
      ...
      <Dialog open={isVideoDemoOpen} onOpenChange={setIsVideoDemoOpen}>
        <DialogContent className="max-w-4xl w-[95vw] p-0 overflow-hidden bg-black/95">
          <DialogHeader className="sr-only">
            <DialogTitle>Demo Recorrido Virtual</DialogTitle>
            <DialogDescription>Video demostrativo de recorrido virtual 360° de propiedades</DialogDescription>
          </DialogHeader>
